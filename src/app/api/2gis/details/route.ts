...
export async function POST(request: NextRequest) {
  try {
    ...
    const phones = contacts.find(c => c.type === 'phone')?.contacts || [];
    const websites = contacts.find(c => c.type === 'website')?.contacts || [];
    const emails = contacts.find(c => c.type === 'email')?.contacts || [];
    ...
    const mainPhoto = photos.find(p => p.type === 'main') || photos[0];
    ...
    const detailedInfo = {
      ...
      contacts: {
        ...
        websites: websites.map(w => w.value),
    ...
    return NextResponse.json(
      { error: 'Ошибка получения данных из 2ГИС', details: error instanceof Error ? error.message : 'Неизвестная ошибка' },
      { status: 500 }
    );
  }
}
